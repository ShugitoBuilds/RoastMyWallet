...
"use client";
  ...
  // Helper to get display name and icon ID
  const getConnectorDisplay = (connector: Connector, index: number, allConnectors: readonly Connector[]) => {
    // Check if it's the second Coinbase connector (Base Wallet)
    const isCoinbase = connector.id === 'coinbaseWalletSDK';
                if (isCoinbase) {
      // Find all coinbase connectors
      const coinbaseConnectors = allConnectors.filter(c => c.id === 'coinbaseWalletSDK');
                // If this is the second one, it's Base Wallet
                if (coinbaseConnectors.indexOf(connector) === 1) {
        return {name: 'Base Wallet', iconId: 'base' };
      }
                return {name: 'Coinbase Wallet', iconId: 'coinbase' };
    }

                if (connector.id === 'injected' || connector.name === 'Injected') {
      return {name: 'Browser Wallet', iconId: 'injected' };
    }

                return {name: connector.name, iconId: connector.id };
  };

  // Filter out duplicate connectors by UID to be safe, but keep same-ID connectors if they are distinct instances
  // Wagmi usually returns distinct instances. We just want to filter if the exact same instance appears twice.
  const uniqueConnectors = connectors.filter((connector, index, self) => 
    index === self.findIndex((c) => c.uid === connector.uid)
                );
                            ...
                            <div className="relative w-full max-w-sm bg-charcoal-900 border border-charcoal-800 rounded-2xl shadow-2xl p-6 space-y-6 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
                                ...
                                <div className="space-y-3">
                                    {uniqueConnectors.map((connector, index) => {
                                        const { name, iconId } = getConnectorDisplay(connector, index, uniqueConnectors);
                               ...
                               hover:bg-charcoal-800 hover:border-flame-500/30 transition-all duration-200 group"
                                            >
                                                <WalletIcon id={iconId} className="w-8 h-8 text-charcoal-300 group-hover:text-flame-400 transition-colors shrink-0" />
